...
export const VplsStaking: React.FC<VplsStakingProps> = ({
  ...
  // vPLS staking uses pool ID 2
  const VPLS_POOL_ID = 2;

  // Check if there's an active unlock
  const hasActiveUnlock = useMemo(() => {
    return Number(vplsUnlockInfo.amount) > 0;
  }, [vplsUnlockInfo.amount]);
  ...
  useEffect(() => {
    ...
    const updateTimer = () => {
      ...
      const unlockAtTimestamp = Number(vplsUnlockInfo.unlockAt);
  ...
  }, [hasActiveUnlock, vplsUnlockInfo.unlockAt]);
  ...
  useEffect(() => {
    const checkApproval = async () => {
      if (
        ...
        const hasAllowance = await checkAllowance(VPLS_POOL_ID, amount);
  ...
  const handleApprove = useCallback(async () => {
    ...
    try {
      ...
      await approve(VPLS_POOL_ID, amount);
  ...
  const handleCancelUnlock = useCallback(async () => {
    ...
    try {
      ...
      await cancelUnlock(VPLS_POOL_ID);
  ...
  const handleFinalizeUnlock = useCallback(async () => {
    ...
    try {
      ...
      await finalizeUnlock(VPLS_POOL_ID);
            ...
            <div className='flex items-center'>
              ...
              <div>
                ...
                <div className='text-[.11rem] text-[#856404] dark:text-[#ffecb5] mt-[.04rem]'>
                  Amount: {formatNumber(vplsUnlockInfo.amount, { decimals: 4 })}{' '}
                  vPLS
                </div>
              </div>
            </div>
          ...
          <div className='flex-1 flex justify-start flex-col pl-[.14rem]'>
            ...
            <div className='mt-[.1rem] text-[.14rem]'>
              <div className='grid grid-cols-2 gap-0'>
                <div></div>
                <div className='text-color-text2 align-middle mt-[7px] mb-[14px]'>
