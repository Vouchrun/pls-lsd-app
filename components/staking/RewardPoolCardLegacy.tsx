...
import { VouchStakingLegacy } from './VouchStakingLegacy';
...
interface RewardPoolCardLegacyProps {
  selectedTab: 'stake' | 'unstake';
  onTabChange: (tab: 'stake' | 'unstake') => void;
}

export const RewardPoolCardLegacy: React.FC<RewardPoolCardLegacyProps> = ({
  selectedTab,
  onTabChange,
}) => {
  const {
  ...
  return (
    <div className=''>
          ...
          <div>
            <p className='text-[18px] font-normal text-color-text1 flex mb-[10px]'>
              ...
              <span className='ml-[10px] px-[10px] py-[2px] pr-[5px] bg-[#FE8A3C] text-[#000] text-[15px] font-normal rounded-[10px] flex gap-[8px] align-middle'>
                Standard Pool 
                <Tooltip title="This pool has been decommissioned. You can only unstake your tokens. For new stakes, use the Vouch Token Staking Pools tab." placement="top" arrow>
                  <span>
      ...
      <VouchStakingLegacy selectedTab={selectedTab} onTabChange={onTabChange} />
